# -*- coding: utf-8 -*-
"""
predict_wav2vec2_mlp.py

Speech emotion recognition using pretrained Wav2Vec2 embeddings
followed by a custom MLP classifier.
Embeddings are extracted from Wav2Vec2 and classified via a PyTorch MLP.

Pipeline:
WAV → Wav2Vec2 → mean pooling → scaler → MLP → emotion prediction

Used by the GUI layer.


Created on Sat Jul 26 22:05:11 2025
@author: tunca
"""
...
model_dir = os.path.join(BASE_DIR, "features_and_models/sota_models_pretrained/wav2vec2_model")
model_name = "facebook/wav2vec2-large-960h-lv60"

# === DEVICE CONFIGURATION ===
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# === LOAD WAV2VEC2 FEATURE EXTRACTOR & MODEL ===
print("📥 Feature extractor loading...")
feature_extractor = AutoFeatureExtractor.from_pretrained(model_name)

print("📥 Wav2Vec2 model loading...")
wav2vec2 = AutoModel.from_pretrained(model_name).to(device)
wav2vec2.eval()

# === LOAD SCALER & LABEL ENCODER ===
print("📦 Scaler and LabelEncoder loadiing...")
...
mlp_model.load_state_dict(torch.load(os.path.join(model_dir, "wav2vec2_mlp_model.pt"), map_location=device))
mlp_model.eval()

print("✅ The model and embedding structures were successfully loaded.")


# === PREDICTION FUNCTION (GUI ENTRY POINT) ===
def predict_emotion(file_path):
    """
    Predict emotion label from a WAV file using
    Wav2Vec2 embeddings + MLP classifier.
    ...
    with torch.no_grad():
        ...
        print("🧠 Wav2Vec2 forward pass completed.")
