# -*- coding: utf-8 -*-
"""
predict_wavlm_mlp.py

Speech emotion recognition using WavLM embeddings
classified by a PyTorch MLP model.

Used by the GUI layer.


Created on Sat Jul 26 19:37:26 2025
@author: tunca
"""
...
model_dir = os.path.join(BASE_DIR, "features_and_models/sota_models_pretrained/wavlm_model")
model_name = "microsoft/wavlm-large"

# === DEVICE CONFIGURATION ===
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# === LOAD FEATURE EXTRACTOR & WAVLM MODEL ===
print("📥 Feature extractor loading...")
feature_extractor = AutoFeatureExtractor.from_pretrained(model_name)

print("📥 WavLM model loading...")
...
mlp_model.load_state_dict(torch.load(os.path.join(model_dir, "wavlm_mlp_model.pt"), map_location=device))
mlp_model.eval()
...
def predict_emotion(file_path):
    ...
    WavLM embeddings + MLP classifier.
    ...
    with torch.no_grad():
        outputs = wavlm(**inputs.to(device))
        print("🧠 WavLM forward pass completed.")
